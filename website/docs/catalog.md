...
ducklink ships **~181 component extensions** (~494 SQL functions; 6 expose
aggregates, 3 require network). Every extension is a Rust `wasm32-wasip2`
component implementing the `duckdb:extension` WIT world. Load one at runtime with
`LOAD <name>` (artifacts in `artifacts/extensions/`), or browse them at
`ducklink serve`. None overlap DuckDB built-ins; each is verified by
`tooling/smoke.py`.

:::note
This page is a curated snapshot. The authoritative, machine-generated index lives
in the repo at `CATALOG.md` (regenerate with `python3 tooling/gen-catalog.py`;
verify integrity with `python3 tooling/verify-catalog.py`). Counts drift as the
catalog grows.
:::
...
- **Aggregates** — `aggstat`, `bloom`, `minhash`, `countmin`, `bitfilters`,
  `tdigest` use the whole-batch `call_aggregate` path.
- **Network** — `dns`, `httpclient`, `openprompt` need an outbound-network grant
  (`DUCKLINK_NETWORK_GRANT`), off by default.

## Loading & embedding

- **Runtime load (every extension):** `LOAD <name>;` pulls
  `artifacts/extensions/<name>.wasm` — no core recompile, version-independent.
  This is the component model's whole point.
- **Static embed (opt-in):** `ducklink compose --embed <name>` bakes an extension
  into the core at build time (wired today for `isin`; `ducklink compose --list`
  shows what's embeddable). Most extensions stay runtime-loaded by design.
- **Network grant:** net extensions are denied by default; opt in with
  `--grant-network all` (or a name allowlist), equivalently the
  `DUCKLINK_NETWORK_GRANT` env var.

See [embedding tracking](guides/embedding-tracking.md) for recording *what builds
embed which extensions*.
...
| **geomtype** | `geom_area`, `geom_astext` | geo, wkt | |
...
| **jsonfns** | `json_valid`, `json_extract`, `json_extract_string`, `json_array_length`, `json_type`, `json_keys`, `json_contains`, `json_quote`, `to_json` | serde_json, serde_json_path | de-embed |
...
| **spatialfns** | `ST_Point`, `ST_GeomFromText`, `ST_AsText`, `ST_X`, `ST_Y`, `ST_Distance`, `ST_Area`, `ST_Length`, `ST_Centroid`, `ST_Contains`, `ST_Within`, `ST_Intersects`, `ST_Envelope`, `ST_AsGeoJSON` | geo, wkt, geojson | de-embed |
| **spatialproj** | `ST_Transform` | geo, wkt | composes `gdal` |
...
| **statsduck** | `lm`, `ttest_1samp`, `ttest_2samp`, `shapiro_wilk`, `mann_whitney_u`, `corr_matrix`, … (31 funcs) | statrs, serde_json | |
...
| **sqlitewasm** | `sqlite_blob_scan` | rusqlite | C-lib-in-wasm |
...
| Extension | Functions | Backed by | Notes |
|---|---|---|---|
| **inetfns** | `host`, `family`, `netmask`, `network`, `broadcast`, `inet_contains` | ipnetwork | de-embed |
...
| **isin** | `isin_validate`, `isin_check_digit`, `isin_country`, `isin_nsin` | hand-rolled | embeddable pilot |
...
| **baseN** | `base32_encode`, `base32_decode`, `base58_encode`, `base58_decode` | base32, bs58 | embeddable pilot |
...
**DuckDB built-ins:** `_comment`, `autocomplete`, `core_functions`, `icu`,
`json`, `parquet`, `tpcds`, `tpch`.

**Official C++ extensions** (static-linked via `EMBED_EXTENSIONS` when selected):
`avro`, `aws`, `azure`, `ducklake`, `excel`, `fts`, `httpfs`, `iceberg`, `inet`,
`mysql_scanner`, `postgres_scanner`, `spatial`, `sqlite_scanner`, `ui`, `vss`.
The eight officials whose surface fits the existing WIT capabilities are now
**componentized** (the de-embed program): the `jsonfns` / `inetfns` / `spatialfns`
de-embed components register the official names directly. See
[the official extensions reference](reference/official-extensions.md).
