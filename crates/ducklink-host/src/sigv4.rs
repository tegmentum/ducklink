//! Hand-rolled AWS Signature Version 4 for S3 requests.
//!
//! Reused verbatim from `extensions/s3fs-component/src/sigv4.rs` (the wasm
//! s3fs transport) so the native-host replicator signs with the exact same,
//! AWS-test-vector-verified primitives instead of reimplementing SigV4 or
//! pulling in the heavy `aws-sigv4` / `aws-smithy` stack. `sign_v4` is generic
//! over the HTTP method + payload hash, so the same code signs the replicator's
//! PUT/GET object requests.
//!
