...
pub enum WriteRoute {
    ...
    /// against an attached foreign catalog. `columns` preserves declaration
    /// order; `type_name` is the raw SQL type-text (e.g. "INTEGER", "TEXT")
    /// with trailing constraints (PRIMARY KEY, NOT NULL, UNIQUE, DEFAULT ...)
    /// stripped. The storage extension maps each type to its native column
    /// type via the dispatch surface.
    CreateTable {
        alias: String,
        table: String,
