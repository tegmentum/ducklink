...
| **avro** | DuckDB's avro-c fork + jansson + snappy | **working** — `read_avro('…')` verified with **deflate + snappy** manifests. Needs DuckDB's **fork** `duckdb/duckdb-avro-c` (the field-id API; stock apache/avro lacks it) + jansson + snappy, built for wasi by `scripts/build-wasi-deps.sh` (lzma/zstd still off). |
...
| **iceberg** | avro extension + roaring; AWS SDK skipped | **working** — `iceberg_scan('table', allow_moved_paths=true)` reads real Iceberg tables (metadata JSON → avro manifests → parquet data), local **and** remote (HTTP/S3) verified. **REST catalog** works too: `ATTACH … (TYPE ICEBERG, ENDPOINT, AUTHORIZATION_TYPE 'none')` and bearer-token via `CREATE SECRET (TYPE ICEBERG, TOKEN …)` — covers R2/Lakekeeper/Polaris/Tabular. AWS SDK avoided by extending the upstream `Emscripten` guards to `WASI` + stubbing `AWSInput` Head/Delete/Post (AWS Glue/S3Tables SigV4 catalogs unsupported). Needs `roaring` + the avro extension. |
