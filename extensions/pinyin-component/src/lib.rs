//! Chinese Hanzi -> pinyin romanization for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus one
//! `datalink_extcore::duckdb_shim!` invocation. The romanization logic + the
//! capability surface (to_pinyin / to_pinyin_plain / to_pinyin_initials) live
//! ONCE in `pinyin-core` (datalink); the registration ABI, handle table, the
