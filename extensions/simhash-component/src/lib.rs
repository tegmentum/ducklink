...
fn scalar(
    ...
    if handle == 1 {
        match text_arg(&args, 0) {
            Some(s) => Ok(types::Duckvalue::Uint64(simhash(&s))),
            None => Ok(types::Duckvalue::Null),
        }
    } else {
        match (text_arg(&args, 0), text_arg(&args, 1)) {
            (Some(a), Some(b)) => Ok(types::Duckvalue::Int64(
                (simhash(&a) ^ simhash(&b)).count_ones() as i64,
            )),
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("SimHash fingerprint".into()),
            tags: vec!["hash".into()],
            attributes: det,
        }),
    )?;
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("SimHash Hamming distance".into()),
            tags: vec!["hash".into()],
            attributes: det,
        }),
    )?;
