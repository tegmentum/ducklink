//! Pig Latin for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus one
//! `datalink_extcore::duckdb_shim!` invocation. The logic + capability surface
//! live ONCE in `piglatin-core` (datalink); the registration ABI, handle table,
//! the dispatch arms (incl. the major-4 columnar hot path), and the `Duckvalue`
