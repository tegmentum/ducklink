//! `tera` scalars for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus
//! one `datalink_extcore::duckdb_shim!` invocation. All logic + the
//! capability surface live ONCE in `tera-core` (datalink); the registration
//! ABI, handle table, the `call_*` arms, and the `Duckvalue` marshalling are
