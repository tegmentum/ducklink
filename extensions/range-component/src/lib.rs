//! Range-from-to TVF for DuckDB — smoke-test component for the
//! datalink table-capability pull-up (T6).
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus
//! one `datalink_extcore::duckdb_shim!` invocation. All logic + the
//! capability surface (the `range_from_to` table-valued function) live
//! ONCE in `range-core` (datalink); the registration ABI, handle table,
//! the six `call_*` arms, and the `Duckvalue` marshalling are derived
