...
fn scalar(
    ...
    let info = match arg(&args, 0).and_then(|s| whatlang::detect(&s)) {
        Some(i) => i,
...
fn one(
    ...
    handlers().lock().unwrap().insert(h, w);
    reg.register(
        name,
        &[runtime::Funcarg {
            name: Some("text".into()),
        ...
        Some(&runtime::Funcopts {
            description: Some("language detection".into()),
            tags: vec!["nlp".into()],
            attributes: attr,
        }),
    )?;
...
fn handlers() -> &'static Mutex<HashMap<u32, W>> {
