...
fn parse_response(raw: &[u8]) -> Option<(u16, std::string::String)> {
    ...
    let body = if chunked {
        dechunk(body).unwrap_or_else(|| body.to_string())
    } else {
        body.to_string()
    };
    Some((status, body))
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("OpenAI-compatible chat completion (model from OPENAI_MODEL)".into()),
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("OpenAI-compatible chat completion (explicit model)".into()),
            tags: vec!["network".into()],
            attributes: net,
        }),
    )?;
