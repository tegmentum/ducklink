...
fn column_defs(table: &str) -> Result<Vec<std::string::String>, String> {
    ...
    for line in rows.lines().filter(|l| !l.trim().is_empty()) {
        ...
        if let (Some(name), Some(ty)) = (name, ty) {
            let notnull = if nullable == Some("NO") {
                " NOT NULL"
            } else {
                ""
            };
    ...
    fn list_commands() -> Vec<CommandSpec> {
        ...
        vec![
            ...
            c(
                FID_CREATE_TABLE,
                "create_table",
                "Create a table from a name:type colspec",
            ...
            c(
                FID_TRANSFORM,
                "transform",
                "Alter columns (rename/drop/retype)",
            ...
            c(
                FID_ADD_FK,
                "add_fk",
                "Add a foreign key (rebuilds the table)",
                "add_fk TABLE COLUMN OTHER_TABLE [OTHER_COLUMN]",
            ),
            c(
                FID_ADD_FKS,
                "add_fks",
                "Add several foreign keys in one rebuild",
                "add_fks TABLE COL:OTHER[:OTHERCOL] ...",
            ),
            c(
                FID_EXTRACT,
                "extract",
                "Normalize column(s) into a lookup table",
    ...
    fn invoke(id: u64, args: String) -> Result<InvokeResult, String> {
        ...
        match id {
            ...
            FID_CREATE_TABLE => {
                ...
                if t.len() < 2 {
                    return Err(
                        "usage: .create_table NAME COL:TYPE [COL:TYPE ...] [--pk COL]".into(),
                    );
                }
                let name = t[0];
                ...
                while i < t.len() {
                    ...
                    let (col, ty) = t[i]
                        .split_once(':')
                        .ok_or_else(|| format!("bad colspec '{}', expected COL:TYPE", t[i]))?;
            ...
            FID_CREATE_INDEX => {
                ...
                let table = t[0];
                let cols: Vec<std::string::String> =
                    t[1..].iter().map(|c| quote_ident(c)).collect();
            ...
            FID_TRANSFORM => {
                ...
                let table = t
                    .first()
                    .ok_or("usage: .transform TABLE [--rename A:B] [--drop C] [--type C:TYPE]")?;
                let qtable = quote_ident(table);
                ...
                while i < t.len() {
                    ...
                    let sql = match flag {
                        "--rename" => {
                            ...
                            format!(
                                "ALTER TABLE {qtable} RENAME COLUMN {} TO {}",
                                quote_ident(a),
                                quote_ident(b)
                            )
                        }
                        ...
                        "--type" => {
                            ...
                            format!(
                                "ALTER TABLE {qtable} ALTER COLUMN {} TYPE {}",
                                quote_ident(c),
                                map_type(ty)
                            )
                        }
            ...
            FID_ADD_FKS => {
                ...
                let table = t
                    .first()
                    .ok_or("usage: .add_fks TABLE COL:OTHER[:OTHERCOL] ...")?;
                if t.len() < 2 {
            ...
            FID_EXTRACT => {
                ...
                if t.len() < 2 {
                    return Err(
                        "usage: .extract TABLE COL [COL ...] [--table LOOKUP] [--fk-column FK]"
                            .into(),
                    );
                }
                let table = t[0];
                ...
                while i < t.len() {
                    match t[i] {
                        "--table" => {
                            lookup = t.get(i + 1).copied();
                            i += 2;
                        }
                        "--fk-column" => {
                            fk_col = t.get(i + 1).copied();
                            i += 2;
                        }
                        c => {
                            cols.push(c);
                ...
                let fk_col = fk_col
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| format!("{lookup}_id"));
                let qtable = quote_ident(table);
                ...
                for c in &cols {
                    spi::query(&format!(
                        "ALTER TABLE {qtable} DROP COLUMN {}",
                        quote_ident(c)
                    ))?;
                }
                Ok(note(format!(
                    "extracted {} into {lookup} (fk {fk_col})",
                    cols.join(", ")
                )))
            }
