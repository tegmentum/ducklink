...
fn key(args: &[types::Duckvalue], i: usize) -> u64 {
    match args.get(i) {
        Some(types::Duckvalue::Int64(n)) => *n as u64,
...
fn scalar_batch_col(
    ...
    let texts: Option<&Vec<String>> = match &textcol.data {
        col::Column::Text(v) => Some(v),
        _ => None,
    };
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("keyed SipHash-1-3".into()),
            tags: vec!["hash".into()],
            attributes: det,
        }),
    )?;
