//! UUID extras for DuckDB (complementing the built-in v4 `uuid()`).
//!
...
//! surface live ONCE in `uuidx-core` (datalink); the registration ABI, the
...
//! `uuid_v7` is declared nondeterministic so the optimizer never folds it.
