...
fn scalar(
    ...
    let bib = match Bibliography::parse(&src) {
        Ok(b) => b,
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    handlers().lock().unwrap().insert(h, B::Count);
    reg.register(
        "bibtex_count",
        &bib_arg(),
        ...
        Some(&runtime::Funcopts {
            description: Some("Number of BibTeX entries; NULL on parse error".into()),
            tags: vec!["bibtex".into()],
            attributes: det,
        }),
    )?;
    ...
    for (name, b, desc) in [
        (
            "bibtex_to_json",
            B::ToJson,
            "BibTeX -> JSON array [{type,key,fields}]; NULL on parse error",
        ),
        (
            "bibtex_keys",
            B::Keys,
            "BibTeX -> JSON array of citation keys; NULL on parse error",
        ),
    ] {
        let h = NEXT.fetch_add(1, Ordering::Relaxed);
        handlers().lock().unwrap().insert(h, b);
        reg.register(
            name,
            &bib_arg(),
...
fn handlers() -> &'static Mutex<HashMap<u32, B>> {
