//! `jsonata` scalar for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus
//! one `datalink_extcore::duckdb_shim!` invocation. All logic + the
//! capability surface live ONCE in `jsonata-core-ext` (datalink); the
//! registration ABI, handle table, the `call_*` arms, and the `Duckvalue`
