...
fn scalar(
    ...
    if which == F::Valid {
        return Ok(types::Duckvalue::Boolean(
            parse(&args)
                .map(|n| phonenumber::is_valid(&n))
                .unwrap_or(false),
        ));
    }
    let n = match parse(&args) {
        Some(n) => n,
    ...
    Ok(match which {
        ...
        F::Format => {
            let mode = match text_arg(&args, 2)
                .unwrap_or_default()
                .trim()
                .to_ascii_lowercase()
                .as_str()
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("valid phone number?".into()),
            tags: vec!["phone".into()],
            attributes: det,
        }),
    )?;
    let h = NEXT.fetch_add(1, Ordering::Relaxed);
    handlers().lock().unwrap().insert(h, F::Format);
    reg.register(
        "phone_format",
        &[
            runtime::Funcarg {
        ...
        runtime::ScalarCallback::new(h),
        Some(&runtime::Funcopts {
            description: Some("format phone number".into()),
            tags: vec!["phone".into()],
            attributes: det,
        }),
    )?;
    let h = NEXT.fetch_add(1, Ordering::Relaxed);
    handlers().lock().unwrap().insert(h, F::Code);
    reg.register(
        "phone_country_code",
        &[
            runtime::Funcarg {
        ...
        runtime::ScalarCallback::new(h),
        Some(&runtime::Funcopts {
            description: Some("country calling code".into()),
            tags: vec!["phone".into()],
            attributes: det,
        }),
    )?;
