//! Fuzzy string matching for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus one
//! `datalink_extcore::duckdb_shim!` invocation. The matching logic + the
//! capability surface (fuzz_ratio / damerau_levenshtein / indel / osa) live ONCE
//! in `rapidfuzz-core` (datalink); the registration ABI, handle table, the
