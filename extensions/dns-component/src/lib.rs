...
fn resolve(host: &str) -> Option<std::vec::Vec<std::string::String>> {
    ...
    let mut ips: std::vec::Vec<std::string::String> = (host, 0u16)
        .to_socket_addrs()
        .ok()?
...
impl callback_dispatch::Guest for Extension {
    ...
    fn call_scalar(
        ...
        let host = match host_arg(&args) {
            Some(s) => s,
            None => return Ok(types::Duckvalue::Null),
        };
        let ips = match resolve(&host) {
            Some(v) => v,
            None => return Ok(types::Duckvalue::Null),
        };
        Ok(if handle == 1 {
    ...
    fn call_table(
        ...
        Err(types::Duckerror::Unsupported("dns: no table fns".into()))
    }
    fn call_pragma(
        _h: u32,
        _a: Vec<types::Duckvalue>,
    ) -> Result<Option<types::Duckvalue>, types::Duckerror> {
        Err(types::Duckerror::Unsupported("dns: no pragmas".into()))
    }
    fn call_cast(_h: u32, _v: types::Duckvalue) -> Result<types::Duckvalue, types::Duckerror> {
        Err(types::Duckerror::Unsupported("dns: no casts".into()))
    }
}
export!(Extension);
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("resolve host -> IP".into()),
    ...
    reg.register(
        ...
        Some(&runtime::Funcopts {
            description: Some("resolve host -> all IPs (JSON)".into()),
            tags: vec!["network".into()],
            attributes: net,
        }),
    )?;
