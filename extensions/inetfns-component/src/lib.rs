...
fn inet_arg(args: &[types::Duckvalue], i: usize) -> Option<Inet> {
    match args.get(i) {
        Some(types::Duckvalue::Text(s)) => parse_inet(s),
        _ => None,
    }
}
...
fn broadcast_addr(addr: &IpAddr, prefix: u8) -> IpAddr {
    match addr {
        IpAddr::V4(a) => {
            let hostmask: u32 = if prefix == 0 {
                u32::MAX
            } else if prefix == 32 {
                0
            } else {
                u32::MAX >> prefix as u32
            };
            IpAddr::V4(Ipv4Addr::from(u32::from(*a) | hostmask))
        }
        IpAddr::V6(a) => {
            let hostmask: u128 = if prefix == 0 {
                u128::MAX
            } else if prefix == 128 {
                0
            } else {
                u128::MAX >> prefix as u32
            };
...
fn contains(net: &Inet, ip: &Inet) -> Option<bool> {
    if net.addr.is_ipv4() != ip.addr.is_ipv4() {
        return Some(false);
    }
...
fn register_scalars() -> Result<(), types::Duckerror> {
    ...
    handlers().lock().unwrap().insert(h, F::Contains);
    reg.register(
        "inet_contains",
        &[
            runtime::Funcarg {
        ...
        Some(&runtime::Funcopts {
            description: Some("does network contain ip".into()),
...
fn one(
    reg: &runtime::ScalarRegistry,
    name: &str,
    ret: types::Logicaltype,
    attr: types::Funcflags,
    f: F,
...
mod tests {
    ...
    fn netmask_v6() {
        ...
        assert_eq!(
            netmask_addr(false, 128).to_string(),
    ...
    fn broadcast_sets_host_bits() {
        let n = inet("192.168.0.5/24");
        assert_eq!(
            broadcast_addr(&n.addr, n.prefix).to_string(),
            "192.168.0.255"
        );
        let n2 = inet("10.1.2.130/25");
        assert_eq!(
            broadcast_addr(&n2.addr, n2.prefix).to_string(),
            "10.1.2.255"
        );
        let host = inet("192.168.0.5/32");
        assert_eq!(
            broadcast_addr(&host.addr, host.prefix).to_string(),
            "192.168.0.5"
        );
    }
    ...
    fn contains_within_and_outside() {
        assert_eq!(
            contains(&inet("192.168.0.0/24"), &inet("192.168.0.5")),
            Some(true)
        );
        assert_eq!(
            contains(&inet("192.168.0.0/24"), &inet("192.168.1.5")),
            Some(false)
        );
        assert_eq!(
            contains(&inet("10.0.0.0/8"), &inet("10.255.255.255")),
            Some(true)
        );
        // mixed families never contain
        assert_eq!(
            contains(&inet("192.168.0.0/24"), &inet("2001:db8::1")),
            Some(false)
        );
        // v6
        assert_eq!(
            contains(&inet("2001:db8::/32"), &inet("2001:db8:1234::1")),
            Some(true)
        );
        assert_eq!(
            contains(&inet("2001:db8::/32"), &inet("2001:db9::1")),
            Some(false)
        );
    }
