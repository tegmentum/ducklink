//! Password-strength scoring for DuckDB.
//!
//! THIN, GENERATED ducklink shim: a `wit_bindgen::generate!` block plus one
//! `datalink_extcore::duckdb_shim!` invocation. The scoring logic + the
//! capability surface (password_score / password_strength) live ONCE in
//! `pwstrength-core` (datalink); the registration ABI, handle table, the
